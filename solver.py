...
def _m3ac1_install():
    try:
        ...
        pass
_m3ac1_install()
from db1d35_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class Db1d35Solver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz132(state):
            amt, rp, tin, tout = _dz130(state)
            _r_dz131 = _dz131()
            return (_r_dz131, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz131, amt, rp, tin, tout = _dz132(state)
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz127(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz126(state):
            amt, dst, rp, src, tin, tout = _dz120(state)
            _r_dz123 = _dz123()
            return (_r_dz123, amt, dst, rp, src, tin, tout)

        def _dz125(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz123, amt, dst, rp, src, tin, tout = _dz126(state)
            if _r_dz123 is not _DR_UNSET:
                return _r_dz123[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz125(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz127(dst, recip, seeded, tout)
            else:
                _dz121()
            _r_dz119, legs = _dz122(dest_ix, dst, src)
    ...
    def _deltas(cls):
        if cls._DELTAS is None:
            ...
            try:
                ...
                cls._DELTAS = {}
        return cls._DELTAS
SOLVER_CLASS = Db1d35Solver
_MINROUTER_FP = 'round-e29781476-n1-min-hk8-cj117-001'
