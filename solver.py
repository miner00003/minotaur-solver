...
class Bg124Solver(_Base):
    ...
    def metadata(self):
        ...
        if SolverMetadata is None:
            return base
        import os as _os
        return SolverMetadata(name=_os.environ.get('MINOTAUR_SOLVER_NAME', 'falcon'), version=_os.environ.get('MINOTAUR_SOLVER_VERSION', '700.46.2'), author=_os.environ.get('MINOTAUR_SOLVER_AUTHOR', 'randy707'), description='champion verbatim + zero-RPC fill-only-empty covers (census + harvested exact-key rows)', supported_chains=base.supported_chains, supported_intent_types=base.supported_intent_types)
...
try:
    ...
    pass
from db6f25_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class Db6f25Solver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz125(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz124(state):
            amt, dst, rp, src, tin, tout = _dz118(state)
            _r_dz121 = _dz121()
            return (_r_dz121, amt, dst, rp, src, tin, tout)

        def _dz123(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz121, amt, dst, rp, src, tin, tout = _dz124(state)
            if _r_dz121 is not _DR_UNSET:
                return _r_dz121[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz123(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz125(dst, recip, seeded, tout)
            else:
                _dz119()
            _r_dz117, legs = _dz120(dest_ix, dst, src)
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz129, amt, rp, tin, tout = _dz130(state)
...
SOLVER_CLASS = Db6f25Solver
_MINROUTER_FP = 'round-e29780803-n1-min-hk8-cj117-001'
