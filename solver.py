...
def _g_hkey(state):
    ...
    p = dict(getattr(state, 'raw_params', None) or {})
    try:
        amt, chain, tin, tout = _dz290(p, state)
    except (TypeError, ValueError):
        return None
...
_mount_g2_overlay()
from d9922f_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D9922fSolver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def metadata(self):
        ...
        try:
            ...
            pass
        return m
SOLVER_CLASS = D9922fSolver
_MINROUTER_FP = 'round-e29772887-n1-min-hk8-cj117-001'
