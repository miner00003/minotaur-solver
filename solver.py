...
def _g_hkey(state):
    ...
    p = dict(getattr(state, 'raw_params', None) or {})
    try:
        amt, chain, tin, tout = _dz307(p, state)
    except (TypeError, ValueError):
        return None
...
from d68ce0_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D68ce0Solver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz300(state):
            amt, rp, tin, tout = _dz298(state)
            _r_dz299 = _dz299()
            return (_r_dz299, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz299, amt, rp, tin, tout = _dz300(state)
            ...
            return None
SOLVER_CLASS = D68ce0Solver
_MINROUTER_FP = 'round-e29774078-n1-min-hk8-cj117-001'
