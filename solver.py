...
def _m3ac1_install():
    try:
        ...
        pass
_m3ac1_install()
from dd068c_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class Dd068cSolver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz101(state):
            amt, rp, tin, tout = _dz99(state)
            _r_dz100 = _dz100()
            return (_r_dz100, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz100, amt, rp, tin, tout = _dz101(state)
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz96(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz95(state):
            amt, dst, rp, src, tin, tout = _dz89(state)
            _r_dz92 = _dz92()
            return (_r_dz92, amt, dst, rp, src, tin, tout)

        def _dz94(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz92, amt, dst, rp, src, tin, tout = _dz95(state)
            if _r_dz92 is not _DR_UNSET:
                return _r_dz92[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz94(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz96(dst, recip, seeded, tout)
            else:
                _dz90()
            _r_dz88, legs = _dz91(dest_ix, dst, src)
    ...
    def metadata(self):
        ...
        try:
            ...
            pass
        return m
SOLVER_CLASS = Dd068cSolver
_MINROUTER_FP = 'round-e29780483-n1-min-hk8-cj117-001'
