...
from d56b5c_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D56b5cSolver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz306(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz305(state):
            amt, dst, rp, src, tin, tout = _dz299(state)
            _r_dz302 = _dz302()
            return (_r_dz302, amt, dst, rp, src, tin, tout)

        def _dz304(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz302, amt, dst, rp, src, tin, tout = _dz305(state)
            if _r_dz302 is not _DR_UNSET:
                return _r_dz302[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz304(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz306(dst, recip, seeded, tout)
            else:
                _dz300()
            _r_dz298, legs = _dz301(dest_ix, dst, src)
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz311(state):
            amt, rp, tin, tout = _dz309(state)
            _r_dz310 = _dz310()
            return (_r_dz310, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz310, amt, rp, tin, tout = _dz311(state)
    ...
    def _eth_url(self):
        ...
        if _r_dz314 is not _DR_UNSET:
            return _r_dz314[0]
SOLVER_CLASS = D56b5cSolver
_MINROUTER_FP = 'round-e29774205-n1-min-hk8-cj117-001'
