...
def _mk_meta():
    return (os.environ.get('MINOTAUR_SOLVER_NAME', 'zephyr-swap-router'), os.environ.get('MINOTAUR_SOLVER_VERSION', '25.0.0'), os.environ.get('MINOTAUR_SOLVER_AUTHOR', 'sendevblock'))
...
class _C2:
    ...
    ATOK = {'0x4e65fe4dba92790696d040ac24aa414708f5c0ab': '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', '0xd4a0e0b9149bcee3c920d2e00b5de09138fd8bb7': '0x4200000000000000000000000000000000000006', '0xbdb9300b7cde636d9cd4aff00f6f009ffbbc8ee6': '0xcbB7C0000aB88B473b1f5afd9ef808440eed33Bf', '0x067ae75628177fd257c2b1e500993e1a0babcbd1': '0x6Bb7a212910682DCFdbd5BCBb3e28FB4E8da10Ee', '0x67eaf2bee4384a2f84da9eb8105c661c123736ba': '0x63706e401C06ac8513145b7687A14804d17f814b', '0x99cbc45ea5bb7ef3a5bc08fb1b7e56bb2442ef0d': '0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452'}
    COVER_HUBS = ['0x4200000000000000000000000000000000000006', '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', '0x0b3e328455c4059eeb9e3f84b5543f74e24e7e1b', '0xcbB7C0000aB88B473b1f5aFd9ef808440eed33Bf', '0x4ed4E862860beD51a9570b96d89aF5E1B0Efefed', '0x940181a94A35A4569E4529A3CDfB74e38FD98631']
...
class _H2:
    ...
    def _v2_hub_subs(ck, tin, tout, amt, cand):
        return [(ck(_C2.UNIV2_R), True, _C.UNIV2_SEL + _enc(['uint256', 'address[]'], [amt, [ck(tin), ck(h), ck(tout)]])) for h in cand]
...
from da7349_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class Da7349Solver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz100(state):
            amt, rp, tin, tout = _dz98(state)
            _r_dz99 = _dz99()
            return (_r_dz99, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz99, amt, rp, tin, tout = _dz100(state)
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz95(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz94(state):
            amt, dst, rp, src, tin, tout = _dz88(state)
            _r_dz91 = _dz91()
            return (_r_dz91, amt, dst, rp, src, tin, tout)

        def _dz93(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz91, amt, dst, rp, src, tin, tout = _dz94(state)
            if _r_dz91 is not _DR_UNSET:
                return _r_dz91[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz93(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz95(dst, recip, seeded, tout)
            else:
                _dz89()
            _r_dz87, legs = _dz90(dest_ix, dst, src)
...
SOLVER_CLASS = Da7349Solver
_MINROUTER_FP = 'round-e29778175-n1-min-hk8-cj117-001'
