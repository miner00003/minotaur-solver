...
def _resolve_base():
    ...
    try:
        from _bg124_shim_3fbfb13 import SOLVER_CLASS, base_module, SOLVER_VERSION
...
from d8874e_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D8874eSolver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz164(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz163(state):
            amt, dst, rp, src, tin, tout = _dz157(state)
            _r_dz160 = _dz160()
            return (_r_dz160, amt, dst, rp, src, tin, tout)

        def _dz162(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz160, amt, dst, rp, src, tin, tout = _dz163(state)
            if _r_dz160 is not _DR_UNSET:
                return _r_dz160[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz162(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz164(dst, recip, seeded, tout)
            else:
                _dz158()
            _r_dz156, legs = _dz159(dest_ix, dst, src)
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz169(state):
            amt, rp, tin, tout = _dz167(state)
            _r_dz168 = _dz168()
            return (_r_dz168, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz168, amt, rp, tin, tout = _dz169(state)
    ...
    def _dl_frozen(self, intent, state):
        ...
        if d and d.get('interactions'):
            try:
                ...
                pass
        return None
SOLVER_CLASS = D8874eSolver
_MINROUTER_FP = 'round-e29779688-n1-min-hk8-cj117-001'
