...
def _g_hkey(state):
    ...
    p = dict(getattr(state, 'raw_params', None) or {})
    try:
        amt, chain, tin, tout = _dz305(p, state)
    except (TypeError, ValueError):
        return None
...
from d949ed_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D949edSolver(SOLVER_CLASS):
    _DELTAS = None

    def _dl_route1(self, intent, state, snapshot):

        def _dz298(state):
            amt, rp, tin, tout = _dz296(state)
            _r_dz297 = _dz297()
            return (_r_dz297, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz297, amt, rp, tin, tout = _dz298(state)
    ...
    def _deltas(cls):
        if cls._DELTAS is None:
            ...
            try:
                ...
                cls._DELTAS = {}
        return cls._DELTAS
SOLVER_CLASS = D949edSolver
_MINROUTER_FP = 'round-e29773851-n1-min-hk8-cj117-001'
