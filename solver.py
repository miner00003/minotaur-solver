...
def _dz102():
    ...
    return (_V2ROUTER, _V3ROUTER, _WETH, _STABLES, SOLVER_NAME, SOLVER_VERSION, SOLVER_AUTHOR)
_V2ROUTER, _V3ROUTER, _WETH, _STABLES, SOLVER_NAME, SOLVER_VERSION, SOLVER_AUTHOR = _dz102()
...
class ForkV2orV3Fill(_Base):
    ...
    def generate_plan(self, intent, state, snapshot=None):
        ...
        def _dz101(state):
            amt, p, quoted, tin, tout = _dz97(state)
            _r_dz98 = _dz98()
            return (_r_dz98, amt, p, quoted, tin, tout)
        ...
        if _r_dz99 is not _DR_UNSET:
            return _r_dz99[0]
        try:
            _r_dz98, amt, p, quoted, tin, tout = _dz101(state)
            if _r_dz98 is not _DR_UNSET:
                return _r_dz98[0]
            _r_dz96, recip = _dz100(p, state)
    ...
    def _v2_path(self, intent, state, tin, tout, amt, min_out, recip):
        ...
        def _dz94(amt, min_out, recip, tin, tout):
        ...
        from common.abi_utils import encode_approve
        from minotaur_subnet.shared.types import Interaction as _IX, ExecutionPlan as _EP
        _r_dz93, params, path = _dz94(amt, min_out, recip, tin, tout)
...
def _dz103():
    ...
    return (WIN_MARGIN_BPS, _COV_MAX_ROWS, _COVER_NAME, _COVER_VERSION, _COVER_AUTHOR)
WIN_MARGIN_BPS, _COV_MAX_ROWS, _COVER_NAME, _COVER_VERSION, _COVER_AUTHOR = _dz103()
...
from df8c60_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_eth_ix

class Df8c60Solver(SOLVER_CLASS):
    _DELTAS = None
    _RESCUE = None
    _OVR = None
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            amt, rp, tin, tout = _dz72(self, state)
            ...
            try:
                ...
                base = None
            _both_hub, _r_dz71, base_ix = _dz73(base, tin, tout)
    ...
    def _dl_census_cover(self, intent, state, rp, tin, tout, amt):
        ...
        if not (c and amt > 0):
            return None
        try:
            _r_dz77, est, pa, po, raw_data, raw_to, recip, route = _dz79(amt, c, rp, state)
    ...
    def _dkey(state):
        try:
            ...
            return ''
SOLVER_CLASS = Df8c60Solver
_MINROUTER_FP = 'round-e29793872-n1-min-hk8-cj117-001'
