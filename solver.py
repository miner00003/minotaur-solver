...
from d0d4ee_router import _dl_os, _dl_json, _DLPlan, _DLIx, _ETH_MAJ, _dl_champ_out, _dl_override

class D0d4eeSolver(SOLVER_CLASS):
    _DELTAS = None
    ...
    def _dl_cross_chain(self, intent, state):
        ...
        def _dz132(dst, recip, seeded, tout):
            dest_ix = [_DLIx(target=tout, value='0', call_data=_xc_transfer(recip, seeded), chain_id=dst)]
            return dest_ix

        def _dz131(state):
            amt, dst, rp, src, tin, tout = _dz125(state)
            _r_dz128 = _dz128()
            return (_r_dz128, amt, dst, rp, src, tin, tout)

        def _dz130(dst, in_cls, rp, seeded):
        ...
        try:
            ...
            _r_dz128, amt, dst, rp, src, tin, tout = _dz131(state)
            if _r_dz128 is not _DR_UNSET:
                return _r_dz128[0]
            in_cls = _xc_class(tin)
            if in_cls is None or dst not in _XC_ROUTER:
                return None
            mapped, recip, seeded = _dz130(dst, in_cls, rp, seeded)
            if str(tout).lower() == str(mapped).lower():
                dest_ix = _dz132(dst, recip, seeded, tout)
            else:
                _dz126()
            _r_dz124, legs = _dz127(dest_ix, dst, src)
    ...
    def _dl_route1(self, intent, state, snapshot):
        ...
        def _dz137(state):
            amt, rp, tin, tout = _dz135(state)
            _r_dz136 = _dz136()
            return (_r_dz136, amt, rp, tin, tout)
        ...
        try:
            if int(getattr(state, 'chain_id', 0) or 0) != 1:
                return None
            _r_dz136, amt, rp, tin, tout = _dz137(state)
    ...
    def _deltas(cls):
        if cls._DELTAS is None:
            ...
            try:
                ...
                cls._DELTAS = {}
        return cls._DELTAS
SOLVER_CLASS = D0d4eeSolver
_MINROUTER_FP = 'round-e29782238-n1-min-hk8-cj117-001'
