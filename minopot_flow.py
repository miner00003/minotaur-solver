...
_MY_BRAND, _MY_AUTHOR, _VERSION_ID, _VERSION, _MARGIN_BPS, _MAJORS, _PSM3, _PSM, _CFG, _HUBS, _FACTORY, _V2_ROUTER_CHAINS = _dz154()
...
def _snap_route(snapshot, tin, tout, amt):
    ...
    if not adj:
        return None
    _r_dz146, ol, tl = _dz147(tin, tout)
...
def _aero_pool_live(w3, cid, tin, tout):
    ...
    from eth_utils import to_checksum_address as CK
    try:
        _r_dz144, gp, pool, r = _dz145(tin, tout, w3)
...
def _alt(s, intent, state, snapshot, base):
    ...
    def _dz142(cid, intent, ix, state):
    ...
    def _dz138(amt, cid, rec, row, tin):
        astable, atoks, call, safe = _dz135(amt, rec, row)
    ...
    amt, cfg, cid, mino, p, tin, tout = _dz134(intent, s, state)
    _r_dz140 = _dz140()
    if _r_dz140 is not _DR_UNSET:
        return _r_dz140[0]
    ol, psm, row, tl = _dz137(cid, tin, tout)
    ...
    if psm:
        ...
        astable, atoks, call, ix, safe = _dz138(amt, cid, rec, row, tin)
        ...
        if not legs:
            return None
        amts, v3_tot = _dz133(a, amt, l, legs)
        ...
        if sr is not None:
            ...
            _dz139()
        path, snap_out = _dz136(sr, v3fees, v3toks)
        ...
        def _quote():
            if w3 is not None:
                try:
                    ...
                    pass
            return snap_out
    _r_dz129, plan = _dz142(cid, intent, ix, state)
...
def _snapshot_path(snapshot, tin, tout):
    ...
    if not ps:
        return None
    edges, ol, orig, tl = _dz125(tin, tout)
...
def _fallback(s, intent, state, snapshot):
    ...
    def _dz111():
        ...
        tokens, fees = tp
        return _DR_UNSET
    amt, cfg, cid, mino, p, tin, tout = _dz113(intent, s, state)
    _r_dz114 = _dz114()
    ...
    from eth_utils import to_checksum_address as CK
    from minotaur_subnet.shared.types import ExecutionPlan, Interaction
    _r_dz112, path = _dz115(fees, tokens)
...
def _cross_chain_plan(s, intent, state, snapshot):
    ...
    def _dz108(intent, state):
        src = int(getattr(state, 'chain_id', 0) or 0)
        dst = _dest_chain(state, intent)
        return (dst, src)

    def _dz107(amt, dst, rec, src, sym, tin):
        bridge_sel, brs, legs, swap_sel = _dz104(amt, dst, rec, src, sym, tin)
        bridged = _bridge_equiv(tin, dst)
        dest_ix = []
        return (bridge_sel, bridged, brs, dest_ix, legs, swap_sel)

    def _dz106(intent, s, state):
        p = _swap_params(s, intent, state)
    ...
    def _dz104(amt, dst, rec, src, sym, tin):
    ...
    dst, src = _dz108(intent, state)
    if not dst or dst == src:
        return None
    amt, p, tin, tout = _dz106(intent, s, state)
    ...
    bridge_sel, bridged, brs, dest_ix, legs, swap_sel = _dz107(amt, dst, rec, src, sym, tin)
