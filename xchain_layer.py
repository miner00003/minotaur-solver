...
def _build_plan(intent, state, src, dst, bridge_token, out_token, bridge_amount, recipient, est_out, src_ix):
    ...
    def _dz157():
        brs = [BridgeRequest(token=bridge_token, amount=int(bridge_amount), src_chain_id=src, dst_chain_id=dst, recipient=recipient, purpose='xchain layer delivery')]
        ccp = CrossChainPlan(legs=legs, bridge_requests=brs)
        return (ExecutionPlan(intent_id=intent.app_id, interactions=[], deadline=int(_t.time()) + 7200, nonce=getattr(state, 'nonce', 0), metadata={'cross_chain_plan': ccp.to_dict(), 'src_chain_id': src, 'dst_chain_id': dst, 'plan_type': 'cross_chain'}),)
        return _DR_UNSET
    import time as _t
    from minotaur_subnet.shared.types import BridgeRequest, ChainLeg, CrossChainPlan, ExecutionPlan
    legs = [ChainLeg(chain_id=src, interactions=list(src_ix), intent_selector='', metadata={'type': 'bridge_source' if not src_ix else 'source_swap'}), ChainLeg(chain_id=dst, interactions=[_transfer_ix(out_token, recipient, est_out, dst)], intent_selector='', metadata={'type': 'destination_swap'})]
    _r_dz157 = _dz157()
    ...
    class XChainSolver(base_cls):
        ...
        def _xc_pure_bridge(self, intent, state, src, dst):
            ...
            def _dz154():
                ...
                return _DR_UNSET
            cap, in_tok, out_tok, rp = _dz155(self, state)
            try:
        ...
        def _xc_source_swap(self, intent, state, snapshot, src, dst):
            ...
            def _dz151():
                ...
                return _DR_UNSET
            cap, in_tok, out_tok, rp = _dz152(self, state)
            try:
                amt = int(rp.get('input_amount') or 0)
            except (TypeError, ValueError):
                amt = 0
        ...
        def _xc_source_swap_leg(self, intent, state, snapshot, src, in_tok, canonical, amt):
            ...
            def _dz148(amt, canonical, in_tok, src, state):
                sub = IntentState(contract_address=_BENCH_EXECUTOR, chain_id=src, nonce=getattr(state, 'nonce', 0), owner=_BENCH_EXECUTOR, raw_params={'input_token': in_tok, 'output_token': canonical, 'input_amount': str(int(amt))}, control={'_intent_function': 'swap'})
                _dz147()
                return sub
            ...
            sub = _dz148(amt, canonical, in_tok, src, state)
            if swap_out <= 0:
                try:
