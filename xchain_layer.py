    ...
    class XChainSolver(base_cls):
        ...
        def _xc_pure_bridge(self, intent, state, src, dst):
            ...
            def _dz167():
                ...
                return _DR_UNSET
            cap, in_tok, out_tok, rp = _dz168(self, state)
            try:
        ...
        def _xc_source_swap(self, intent, state, snapshot, src, dst):
            ...
            def _dz164():
                ...
                return _DR_UNSET
            cap, in_tok, out_tok, rp = _dz165(self, state)
            try:
                amt = int(rp.get('input_amount') or 0)
            except (TypeError, ValueError):
                amt = 0
        ...
        def _xc_source_swap_leg(self, intent, state, snapshot, src, in_tok, canonical, amt):
            ...
            def _dz161(amt, canonical, in_tok, src, state):
            ...
            sub = _dz161(amt, canonical, in_tok, src, state)
            if swap_out <= 0:
                try:
