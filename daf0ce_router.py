...
def _dl_bal_quote(url, tin, tout, amt, pid):
    ...
    def _dz61(amt, pid, tin, tout, url):
        ...
        return (data, r, sig, z)
    from eth_abi import encode
    data, r, sig, z = _dz61(amt, pid, tin, tout, url)
...
def _dl_eth_ix(tin, tout, amt, recipient, route, min_out=1):
    ...
    def _dz51(amt, mo, recipient, route, tin, tout):
        fee = route[1][1]
    ...
    from eth_abi import encode
    amt, approve, kind, mo = _dz50(amt, min_out, route)
    _r_dz53 = _dz53()
    if _r_dz53 is not _DR_UNSET:
        return _r_dz53[0]
    if kind == 'single':
        fee, swap = _dz51(amt, mo, recipient, route, tin, tout)
    else:
        fees, tokens = _dz52(route)
...
def _dl_decode_path(body, sel, url):
    ...
    def _dz45():
        ...
        return _DR_UNSET
    from eth_abi import decode
    _mo, _rec, amt, fees, p, path, toks = _dz46(body, sel)
    _r_dz45 = _dz45()
...
def _dl_decode_one(cd, url, target=None):
    ...
    try:
        if sel == _SEL_EIS_02:
            _m, _r, _s, amt, fee, q, tin, tout = _dz43(body, url)
            return ('ANSWER', q if q > 0 else None)
        _r_dz42 = _dz42()
...
def _dl_univ2_requote(url, router, cd):
    ...
    try:
        ...
        return 0
    _r_dz36, data, r = _dz37(amt, path, router)
...
_XC_CANON, _XC_ROUTER, _XC_ANVIL = _dz62()
