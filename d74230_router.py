...
def _dl_bal_quote(url, tin, tout, amt, pid):
    ...
    def _dz60(amt, pid, tin, tout, url):
        ...
        return (data, r, sig, z)
    from eth_abi import encode
    data, r, sig, z = _dz60(amt, pid, tin, tout, url)
...
def _dl_eth_ix(tin, tout, amt, recipient, route, min_out=1):
    ...
    def _dz50(amt, mo, recipient, route, tin, tout):
        fee = route[1][1]
    ...
    from eth_abi import encode
    amt, approve, kind, mo = _dz49(amt, min_out, route)
    _r_dz52 = _dz52()
    if _r_dz52 is not _DR_UNSET:
        return _r_dz52[0]
    if kind == 'single':
        fee, swap = _dz50(amt, mo, recipient, route, tin, tout)
    else:
        fees, tokens = _dz51(route)
...
def _dl_decode_path(body, sel, url):
    ...
    def _dz44():
        ...
        return _DR_UNSET
    from eth_abi import decode
    _mo, _rec, amt, fees, p, path, toks = _dz45(body, sel)
    _r_dz44 = _dz44()
...
def _dl_decode_one(cd, url, target=None):
    ...
    try:
        if sel == _SEL_EIS_02:
            _m, _r, _s, amt, fee, q, tin, tout = _dz42(body, url)
            return ('ANSWER', q if q > 0 else None)
        _r_dz41 = _dz41()
...
def _dl_univ2_requote(url, router, cd):
    ...
    try:
        ...
        return 0
    _r_dz35, data, r = _dz36(amt, path, router)
...
_XC_CANON, _XC_ROUTER, _XC_ANVIL = _dz61()
