...
def _dr142():
    ...
    def _dr31():
        ...
        SOLVER_AUTHOR, SOLVER_NAME, SOLVER_VERSION, _FAST_DIRECT_INPUTS, _HOLE_SPEND_CAPS, _UR_CONTRACT_BALANCE = _dz247()
...
def _dr82():
    ...
    def _dr330():
        ...
        _ETH_HUBS, _ETH_UNI_FEES, _ETH_UNI_FEES_TWOHOP = _dz245()
...
class _MinerSolverDR10(_MinerSolverDR10DR170):
    ...
    def _shp_uniswap_v4_ur(self, intent, state, snapshot, cand, tin, tout, amount_in, recipient, deadline, chain_id):
        ...
        def _dr201():
            ...
            _dr86()
            if has_v2:
                commands, v2_tokens = _dz227(inputs)
...
class _MinerSolverDR11(_MinerSolverDR11DR171):
    ...
    def _sweep_simulate_one(self, w3, app, tin, tout, amount_in, slot_idx, cand):
        ...
        def _dz241(amount_in, app, deadline, path, router):
            spender = router
        ...
        if kind == 'v2':
            call, spender, target = _dz241(amount_in, app, deadline, path, router)
        else:
        ...
        def _dr305():
            ...
            def _dr108():
                ...
                def _fw16():
                    ...
                    agg3, av2, calc, lk, sp, zero = _dz185()
...
class _MinerSolverDR56(_MinerSolverDR11):
                                                ...
                                                def _dr5():
                                                    ...
                                                    if kind == 'aero_v2':
                                                        ...
                                                        def _dr242():
                                                            ...
                                                            return routes
                                                        cand, routes = _dz150(factory_addr)
                                                    else:
                                                        return None
                                                    return _DR_UNSET
                                                _dr6 = _dr5()
        ...
        try:
            ...
            def _dr277():
                ...
                def _dz209():
                    ...
                    chain_id = int(state.chain_id or (snapshot.chain_id if snapshot else 0) or 0)
                    return (amount_in, chain_id, min_out)
                amount_in, chain_id, min_out = _dz209()
                if chain_id != _BASE or amount_in <= 0 or (not tin) or (not tout):
                    return None
    ...
    def _erc4626_wrap_plan(self, intent, state, snapshot, tin, tout, amount_in, chain_id):
        ...
        try:
            ...
            if tin.lower() == _WETH:
                return None
            uni_router, w3 = _dz236(chain_id, self)
            if w3 is None or not uni_router:
                return None
...
class _MinerSolverDR176(_MinerSolverDR123):
    ...
    def _best_effort_singlehop_plan(self, intent, state, snapshot):
        ...
        params, tin, tout = _dz231(intent, self, state)
        try:
            amount_in = int(params.get('input_amount', 0) or 0)
            amount_in = self._effective_swap_amount(self._fee_params(state, params), tin, amount_in)
        except (TypeError, ValueError):
            ...
            def _dr67():
                ...
                def _dr42():
                    ...
                    def _twohop_mids():
                        ...
                        majors, mids, tin_l, tout_l = _dz165()
