...
def _dl_eth_ix(tin, tout, amt, recipient, route, min_out=1):
    ...
    def _dz80(amt, mo, recipient, route, tin, tout):
        fee = route[1][1]
    ...
    from eth_abi import encode
    amt, approve, kind, mo = _dz79(amt, min_out, route)
    _r_dz82 = _dz82()
    if _r_dz82 is not _DR_UNSET:
        return _r_dz82[0]
    if kind == 'single':
        fee, swap = _dz80(amt, mo, recipient, route, tin, tout)
    else:
        fees, tokens = _dz81(route)
...
def _dl_decode_path(body, sel, url):
    ...
    def _dz74():
        ...
        return _DR_UNSET
    from eth_abi import decode
    _mo, _rec, amt, fees, p, path, toks = _dz75(body, sel)
    _r_dz74 = _dz74()
...
def _dl_decode_one(cd, url, target=None):
    ...
    try:
        if sel == _SEL_EIS_02:
            _m, _r, _s, amt, fee, q, tin, tout = _dz72(body, url)
            return ('ANSWER', q if q > 0 else None)
        _r_dz71 = _dz71()
...
def _dl_univ2_requote(url, router, cd):
    ...
    try:
        ...
        return 0
    _r_dz65, data, r = _dz66(amt, path, router)
